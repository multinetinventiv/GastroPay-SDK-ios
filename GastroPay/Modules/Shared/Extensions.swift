...
public extension String {
    ...
    func formatNumbers(withMask mask: String = "0 (XXX) XXX XX XX") -> String {
        ...
        var result = ""
        var index = numbers.startIndex ***REMOVED*** numbers iterator

        ***REMOVED*** iterate over the mask characters until the iterator of numbers ends
        for ch in mask where index < numbers.endIndex {
            if ch == "X" {
                guard numbers.indices.contains(index) else { continue }

                ***REMOVED*** mask requires a number in this place, so take the next one
                result.append(numbers[index])

                ***REMOVED*** move numbers iterator to the next index
                index = numbers.index(after: index)

            } else {
                result.append(ch) ***REMOVED*** just append a mask character
            }
        }
        return result
    }
...
public extension UIViewController {
    ...
    func add(childViewController viewController: UIViewController) {
        ...
        ***REMOVED*** Add Child View Controller
        addChild(viewController)

        ***REMOVED*** Add Child View as Subview
        view.addSubview(viewController.view)

        ***REMOVED*** Configure Child View
        viewController.view.frame = view.bounds
        viewController.view.autoresizingMask = [.flexibleWidth, .flexibleHeight]

        ***REMOVED*** Notify Child View Controller
    ...
    func remove(childViewController viewController: UIViewController) {
        ...
        ***REMOVED*** Remove Child View From Superview
