...
public class MICheckBox: UIControl {
    public enum Style {
        ***REMOVED***/ ■
        case square
        ***REMOVED***/ ●
    ...
    ***REMOVED***Used to increase the touchable are for the component
    public var increasedTouchRadius: CGFloat = 5

    ***REMOVED***By default it is true
    ...
    ***REMOVED***UIImpactFeedbackGenerator object to wake up the device engine to provide feed backs
    private var feedbackGenerator: UIImpactFeedbackGenerator?

    ***REMOVED***MARK: Intialisers
    ...
    ***REMOVED***Define the above UIImpactFeedbackGenerator object, and prepare the engine to be ready to provide feedback.
    ***REMOVED***To store the energy and as per the best practices, we create and make it ready on touches begin.
    ...
    ***REMOVED***On touches ended,
    ***REMOVED***change the selected state of the component, and changing *isChecked* property, draw methos will be called
    ***REMOVED***So components appearance will be changed accordingly
    ***REMOVED***Hence the state change occures here, we also sent notification for value changed event for this component.
    ***REMOVED***After usage of feedback generator object, we make it nill.
    open override func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?) {
        ***REMOVED***        super.touchesEnded(touches, with: event)
    ...
    open override func draw(_ rect: CGRect) {
        ...
        ***REMOVED***Draw the outlined component
        ...
        ***REMOVED***When it is selected, depends on the style
        ***REMOVED***By using helper methods, draw the inner part of the component UI.
        if isChecked {
            ...
            case .fullSquareWithTick:
                ...
                shapePath.fill()
                let bezierPath = getCheckMarkBezierPath(frame: newRect)
    ...
    ***REMOVED***we override the following method,
    ***REMOVED***To increase the hit frame for this component
    ***REMOVED***Usaully check boxes are small in our app's UI, so we need more touchable area for its interaction
    ...
    func getCheckMarkBezierPath(frame: CGRect) -> UIBezierPath {
        ***REMOVED******REMOVED*** Bezier Drawing
    ...
    ***REMOVED***Draws tick inside the component
    func drawCheckMark(frame: CGRect) {
        ***REMOVED******REMOVED*** Bezier Drawing
        let bezierPath = getCheckMarkBezierPath(frame: frame)
        checkmarkColor.setFill()
        bezierPath.fill()
    }

    ***REMOVED***Draws circle inside the component
    func drawCircle(frame: CGRect) {
        ***REMOVED******REMOVED*** General Declarations
        ***REMOVED*** This non-generic function dramatically improves compilation times of complex expressions.
        func fastFloor(_ x: CGFloat) -> CGFloat { return floor(x) }

        ***REMOVED******REMOVED*** Oval Drawing
    ...
    ***REMOVED***Draws square inside the component
    func drawInnerSquare(frame: CGRect) {
        ***REMOVED******REMOVED*** General Declarations
        ***REMOVED*** This non-generic function dramatically improves compilation times of complex expressions.
        func fastFloor(_ x: CGFloat) -> CGFloat { return floor(x) }

        ***REMOVED******REMOVED*** Rectangle Drawing
        ...
        ***REMOVED***        let rectanglePath = UIBezierPath(rect: CGRect(x: frame.minX + fastFloor(frame.width * 0.22000 + 0.15), y: frame.minY + fastFloor(frame.height * 0.26000 + 0.15), width: fastFloor(frame.width * 0.76000 + 0.15) - fastFloor(frame.width * 0.22000 + 0.15), height: fastFloor(frame.height * 0.76000 + 0.15) - fastFloor(frame.height * 0.26000 + 0.15)))
        checkmarkColor.setFill()
        rectanglePath.fill()
    }

    ***REMOVED***Draws cross inside the component
    func drawCross(frame: CGRect) {
        ...
        ***REMOVED******REMOVED*** Rectangle 2 Drawing
        context.saveGState()
